  ...
  <div class="flex justify-between max-w-2xl px-4 py-4 mx-auto sm:px-8">
    ...
    <div class="space-x-3 transition text-primary-500">
      <a v-if="appConfig.socials?.twitter" :href="`https://twitter.com/${appConfig.socials?.twitter}`"
        title="Twitter da Softagon" target="_blank"
        class="dark:text-primary-100 hover:text-primary-700 dark:hover:text-primary-300">
        <Icon name="fa-brands:twitter" class="w-5 h-5" />
      </a>
      <a v-if="appConfig.socials?.contact" :href="`${appConfig.socials?.contact}`" title="Site da Softagon"
        target="_blank" class="dark:text-primary-100 hover:text-primary-700 dark:hover:text-primary-300">
        <Icon name="cib:whatsapp" class="w-5 h-5" />
      </a>
      <a v-if="appConfig.socials?.website" :href="`${appConfig.socials?.website}`" title="Fale com a Softagon"
        target="_blank" class="dark:text-primary-100 hover:text-primary-700 dark:hover:text-primary-300">
        <Icon name="game-icons:sea-dragon" class="w-5 h-5" />
      </a>
